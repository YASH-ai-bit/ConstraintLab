...
const validationFailure = (message: string): ActionResult<never> => ({
  ok: false,
  code: "VALIDATION_ERROR",
  message,
  recoverable: true,
  suggestedAction: "Correct the arguments and retry.",
});
...
function compactProblemState() {
  ...
  return {
    ...
    staleSolution:
      state.solvedModelVersion !== undefined &&
      state.solvedModelVersion !== state.modelVersion,
    ...
    infeasibility: state.infeasibility
      ? structuredClone(state.infeasibility)
      : undefined,
  };
}
...
export const toolHandlers = {
  get_problem_state(input: unknown) {
    ...
    return parsed.ok
      ? { ok: true as const, data: compactProblemState() }
      : parsed;
  },
  ...
  add_job(input: unknown) {
    ...
    return parsed.ok
      ? appStore.getState().addJob(parsed.data, "agent")
      : parsed;
  },
  ...
  update_job(input: unknown) {
    ...
    return parsed.ok
      ? appStore
          .getState()
          .updateJob(parsed.data.id, parsed.data.updates, "agent")
      : parsed;
  },
  ...
  add_constraint(input: unknown) {
    ...
    return parsed.ok
      ? appStore.getState().addConstraint(parsed.data, "agent")
      : parsed;
  },
  ...
  remove_constraint(input: unknown) {
    ...
    return parsed.ok
      ? appStore.getState().removeConstraint(parsed.data.id, "agent")
      : parsed;
  },
  ...
  set_objective(input: unknown) {
    ...
    return parsed.ok
      ? appStore.getState().setObjective(parsed.data, "agent")
      : parsed;
  },
  ...
  ): Promise<ActionResult<SolverResult>> {
    const parsed = parse(solveProblemSchema, input ?? {});
    return parsed.ok
      ? appStore.getState().solveProblem("agent", context?.signal)
      : parsed;
  },
  ...
  get_solution(input: unknown) {
    ...
    if (
      state.solvedModelVersion !== undefined &&
      state.modelVersion !== state.solvedModelVersion
    )
      return {
        ok: false as const,
        code: "STALE_SOLUTION",
        message: `The solution is for model v${state.solvedModelVersion}, but the current model is v${state.modelVersion}.`,
        recoverable: true,
        suggestedAction: "Call solve_problem again.",
      };
    if (
      state.solvedModelVersion === undefined ||
      !["OPTIMAL", "FEASIBLE"].includes(state.solveStatus)
    )
      return {
        ok: false as const,
        code: "NO_SOLUTION",
        message: "No solved schedule is available.",
        recoverable: true,
        suggestedAction: "Call solve_problem first.",
      };
    return {
      ok: true as const,
      data: {
        modelVersion: state.solvedModelVersion,
  ...
  analyze_infeasibility(input: unknown) {
    ...
    if (
      ...
      return {
        ok: false as const,
        code: "STALE_INFEASIBILITY",
        message: `The infeasibility result is for model v${state.solvedModelVersion}, but the current model is v${state.modelVersion}.`,
        recoverable: true,
        suggestedAction:
          "Call solve_problem again, then analyze only if the current model is infeasible.",
      };
    if (state.solveStatus !== "INFEASIBLE")
      return {
        ok: false as const,
        code: "MODEL_NOT_INFEASIBLE",
        message: `Current solve status is ${state.solveStatus}.`,
        recoverable: true,
        suggestedAction:
          "Call solve_problem, then analyze only if the result is infeasible.",
      };
